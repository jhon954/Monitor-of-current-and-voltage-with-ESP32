...
#define SAMPLE_ARCHITECTURE_PROPERTY_VALUE             "ESP32 WROOM32"
...
#define TELEMETRY_PROP_NAME_VOLTAGE                      "voltage"
#define TELEMETRY_PROP_NAME_CURRENT                      "current"
...
static int generate_telemetry_payload(uint8_t* payload_buffer, size_t payload_buffer_size, size_t* payload_buffer_length)
{
  ...
  rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR(TELEMETRY_PROP_NAME_VOLTAGE));
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding voltage property name to telemetry payload.");
  rc = az_json_writer_append_double(&jw, voltage, DOUBLE_DECIMAL_PLACE_DIGITS);
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding voltage property value to telemetry payload. ");

  rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR(TELEMETRY_PROP_NAME_CURRENT));
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding current property name to telemetry payload.");
  rc = az_json_writer_append_double(&jw, current, DOUBLE_DECIMAL_PLACE_DIGITS);
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding current property value to telemetry payload. ");
